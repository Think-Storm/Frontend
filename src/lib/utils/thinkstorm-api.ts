...
export const getProjects = async (
  ...
  try {
    ...
    if (technical && technical.length > 0)
      url.searchParams.append('technicalLabels', technical.join(','))

    if (domain && domain.length > 0)
      url.searchParams.append('domainLabels', domain.join(','))

    if (goal) url.searchParams.append('goal', goal)
    if (languageName) url.searchParams.append('languageName', languageName)
    ...
    return json as SearchProjectResponse
  } catch (error) {
    console.error(

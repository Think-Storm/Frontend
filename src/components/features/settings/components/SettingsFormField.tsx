...
import { Control, Path, FieldValues, useFormContext, useForm } from "react-hook-form";
                ...
                <FormItem>
                    <FormControl>
                        <div className="relative w-24 h-24 cursor-pointer">
                            <Input type={type} {...field} ref={ref} className="absolute w-0 h-0 opacity-0" />
                        </div>
                    </FormControl>
                </FormItem>
            )}
...
export default function SettingsTagInputFormField<T extends FieldValues>({
    ...
    const filteredSuggestions = suggestions?.filter(
        (s) => s.toLowerCase().includes(input.toLowerCase()) && !temporaryTags.includes(s)
    );

    const showTags = (tags: string[], kind: string, onRemove: (tag: string) => void) => {
                                ...
                                <Modal ref={modalRef}>
                                    <>
                                        ...
                                        <div className="flex flex-wrap justify-start items-center gap-2 rounded-md bg-background px-2 w-[95%]">
                                            {showTags(temporaryTags, "temporaryTags", removeTemporaryTag)}
                                            {filteredSuggestions && filteredSuggestions.length > 0 && (
                                                <div className="flex flex-wrap items-center gap-2 rounded-md mb-2 w-full">
                                                    <h1 className="font-bold text-lg w-full">Suggestions</h1>
...
export function SettingsLinkInputFormField<T extends FieldValues>({
    ...
    const removeLink = (linkToRemove: { type: string; url: string }) => {
        ...
        setTemporaryLinks(temporaryLinks.filter((l: { url: string }) => l.url !== linkToRemove.url));
    };

    const removeTemporaryLink = (linkToRemoveUrl: string) => {
        setTemporaryLinks(temporaryLinks.filter((l: { url: string }) => l.url !== linkToRemoveUrl));
    };
    ...
    const showLinks = (links: { type: string; url: string }[], url: string, onRemove: (link: string) => void) => {
                    ...
                    <FormControl>
                        <div className="mb-4">
                            ...
                            <Modal ref={modalRef}>
                                <>
                                    <div className="font-bold text-2xl my-3">{title}</div>
                                    <FormLabel className="font-bold text-lg text-left w-[95%]">Select Type</FormLabel>
                                    <select
                                        value={type}
                                    ...
                                    <FormLabel className="font-bold text-lg text-left w-[95%]">Link</FormLabel>
                                    <Input
                                        value={url}

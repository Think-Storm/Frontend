  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4 ">
      {projects.map((project, index) => (
        <div key={project.id} className="p-4 border border-gray-200 rounded-md shadow-md">
          ...
          <div className="flex gap-2 mt-2 flex-wrap">
            {project.technicalLabels.map((tech, i) => (
              <span key={`${project.id}-tech-${i}`} className="text-xs  px-2 py-1 rounded">
                <StackIcon name={tech.labelName} className="w-[18px] h-[18px]" />
              </span>
            ))}
          </div>
        </div>

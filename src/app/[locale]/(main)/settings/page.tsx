...
export default function Settings() {
  ...
  function onSubmitUserProfile(ProfileFormValues: UserProfileData) {
    updateSettings({ kind: "profile", data: ProfileFormValues, id: 3 });
  }
  ...
  function onSubmitDeleteAccount() {
    updateSettings({ kind: "delete-profile", id: 3 });
  }

  const profileData = (
    useProfileSettings(3) as { data?: { data: UserProfileData } }
  )?.data?.data;
  ...
  const handleSaveAll = async () => {
    ...
    await Promise.all([
      updateSettings({ kind: "profile", data: profileValues, id: 3 }),
